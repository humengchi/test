...
{
//    BOOL isPhone = UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone;     //通过用户接口判断是不是iphone
    ...
    [self setRootViewController:loginVC animated:YES];  //设置窗口的root视图控制器为登录试图控制器
}
//
- (void)transitionToMainViewController
{
    ...
    ADRootViewController *mainVC = [[NSClassFromString(vcStr) alloc] initWithNibName:vcStr bundle:nil];//创建用户中心的视图
    mainVC.isCarAssistantVC = @"no";
//    mainVC.updateViewConstraints
...
- (void)translateToViewController:(UIViewController *)viewController  //视图跳转
{
