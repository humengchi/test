...
- (void)viewDidLoad
{
    ...
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, WIDTH, 416) style:UITableViewStylePlain];
    ...
    _infoView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.bounds.size.height - 150, WIDTH, 150)];
...
//    [_infoView addSubview:labelOfDate];
    ...
    if (IOS7_OR_LATER && !DEVICE_IS_IPHONE5) {            //IOS7.0 3.5寸屏幕
        [_tableView setFrame:CGRectMake(0, 64, WIDTH, 416)];
	}
    if (!IOS7_OR_LATER && DEVICE_IS_IPHONE5) {            //IOS6.0 4.0寸屏幕
        [_tableView setFrame:CGRectMake(0, 0, WIDTH, 504)];
    }
    if (IOS7_OR_LATER && DEVICE_IS_IPHONE5) {             //IOS7.0 4.0寸屏幕
        [_tableView setFrame:CGRectMake(0, 64, WIDTH, 504)];
	}
...
{
    [IVToastHUD endRequestWithResultType:IV_TOAST_REQUEST_FIAL message:NSLocalizedStringFromTable(@"code201",@"MyString", @"")];
}
