...
//    [self getPhoneBookFirend];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(getFriendLists:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(addFriendInBook)
                                                 name:@"ADDFRIENDBOOK_NOT"
                                               object:nil];
    ...
    if (IOS7_OR_LATER) {
        ...
        frame.origin.y = 64;
//        frame.size.height = self.view.frame.size.height - 64 - self.tabBarController.view.bounds.size.height;
...
//        TKAddressBook *book = [addressBookTemp objectAtIndex:indexPath.row];
        if(_friendLists.count == 0) return nil;
